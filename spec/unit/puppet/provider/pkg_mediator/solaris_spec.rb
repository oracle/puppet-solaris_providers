require 'spec_helper'
provider_class = Puppet::Type.type(:pkg_mediator).provider(:solaris)
...
describe provider_class do
   ...
   before(:each) do
    ...
    @resource = Puppet::Type.type(:pkg_mediator).new(
      :name => "foo", :ensure => :present, :version => "1.1.1", :implementation => "a@1.2")
  ...
  describe ".instances" do
    ...
    describe "should get a list of mediators" do
      provider_class.expects(:pkg).with(:mediator, "-H", "-F", "tsv").returns File.read(my_fixture('pkg_mediator.txt'))
      instances = provider_class.instances.map { |p| {:name => p.get(:name),
        :ensure => p.get(:ensure), :version => p.get(:version),
      ...
      it "with version and implementation" do
        expect(instances[0]).to eq({:name => 'foo', :ensure => :present, :version => '1.1.1', :implementation => 'a@1.2'})
      end
      it "with version only" do
        expect(instances[1]).to eq({:name => 'fooVer', :ensure => :present, :version => '1.1.1', :implementation => :None})
      end
      it "with implementation only" do
        expect(instances[2]).to eq({:name => 'fooImp', :ensure => :present, :version => :None, :implementation => 'a@1.2'})
      end
    end
  end
end
