...
#      [ :static, :dhcp, :addrconf, :from_gz, :inherited ].each do |address_type|
#        context "#{address_type}" do
#          [:address,:remote_address,:down,:seconds,:hostname,:interface_id,:remote_interface_id].each do |prop|
#            it "with #{prop}"
#          end
#        end
#      end
      context "implied" do
        [[:address,'1.2.3.4'],[:remote_address,'foo'],[:down,'true']].product(
          [[:seconds,5],[:hostname,'bar'],
            [:interface_id,'net0/v4'],[:remote_interface_id,'net1/v4']]).each do |a|
